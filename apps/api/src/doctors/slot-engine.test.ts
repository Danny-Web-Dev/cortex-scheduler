...
import { computeFreeSlots, isFreeSlot, type AvailabilityWindow } from './slot-engine';
...
describe('computeFreeSlots', () => {
  ...
  it('converts clinic-local time to UTC using summer DST offset (IDT, UTC+3)', () => {
    ...
    expect(slots[0]?.startsAt).toBe('2026-07-13T06:00:00.000Z'); // 09:00 IDT = 06:00 UTC
  });

  it('converts clinic-local time to UTC using winter offset (IST, UTC+2)', () => {
    const date = '2026-01-12'; // January → Israel Standard Time, UTC+2
    const slots = computeFreeSlots({
      availability: availabilityFor(date),
      appointments: [],
      date,
    ...
    expect(slots[0]?.startsAt).toBe('2026-01-12T07:00:00.000Z'); // 09:00 IST = 07:00 UTC
  });
  ...
  it('filters out slots already in the past relative to now', () => {
    const date = '2026-07-13';
    // now = midday, so morning slots are gone.
